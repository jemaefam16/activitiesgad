...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `activities` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Update ": "Create New " ?> Activity</h3>
	</div>
	<div class="card-body">
		<form action="" id="act-form">
			<input type="hidden" name ="id" value="<?php echo isset($id) ? $id : '' ?>">
			<div class="form-group">
				<label for="title" class="control-label">Activity Title</label>
                <textarea name="title" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($title) ? $title : ''; ?></textarea>
			</div>
            <div class="form-group">
				<label for="event_location" class="control-label">Activity Location</label>
                <textarea name="event_location" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($event_location) ? $event_location : ''; ?></textarea>
			</div>
            <div class="form-group">
				<label for="label" class="control-label">Label (Open or Close)</label>
                <textarea name="label" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($label) ? $label : ''; ?></textarea>
			</div>
            <div class="form-group">
				<label for="label" class="control-label">Attendance Status (Open or Close)</label>
                <textarea name="attendance_status" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($attendance_status) ? $attendance_status : ''; ?></textarea>
			</div>
            <div class="form-group">
				<label for="label" class="control-label">Evaluation Status (Open or Close)</label>
                <textarea name="evaluation_status" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($evaluation_status) ? $evaluation_status : ''; ?></textarea>
			</div>
            <div class="form-group">
	...
	<div class="card-footer">
		...
		<a class="btn btn-lg btn-default" href="?page=activities" style="background-color:red;color:white">Cancel</a>
	</div>
</div>
<script>
